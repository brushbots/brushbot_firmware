...
"""
    ...
    with open("main.py", "w") as f:
        f.write(
            """\
# This is an autogenerated main which is executed after boot
from ota_updater.ota_updater import OTAUpdater
from main.constants import Constants

o = OTAUpdater('https://github.com/brushbots/brushbot_firmware')

def start_main():
	from main.brushbot_firmware import run_main
	run_main()

OTAUpdater.using_network(Constants.NETWORK_SSID, Constants.NETWORK_PWD) # connect to a network expected to have internet connection
o.download_and_install_update_if_available() # if an update has been previously downloaded, install it
o.check_for_update_to_install_during_next_reboot() # download available update; this check must be done also on response to a command from the brushbotarium server
start_main() # start firmware
"""
        )
    uos.mkdir("main")
    uos.chdir("main")
...
class Constants:
    ...
    NETWORK_PWD = 'brushy1846'
"""
        )

    with open("brushbot_firmware.py", "w") as f:
        f.write(
            """\
import utime
...
"""
    ...
    uos.chdir("..")
    uos.mkdir("ota_updater")
    uos.chdir("ota_updater")
    with open("__init__.py", "w") as f:
        f.write(
            """\
"""
    ...
    with open("ota_updater.py", "w") as f:
        f.write(
            """\
import usocket
import os
